  ...
  try {
    if (!request.user || request.user.role !== "user") {
      return response
        ...
        .json({
          message:
            "Access forbidden: Only authenticated users with role user can access this resource.",
        });
    }
  ...
  try {
    ...
    if (role !== "host") {
      return response
        .status(403)
        .json({
          message: "Unauthorized: Only hosts can submit event proposals.",
        });
    }
    ...
    return response
      .status(201)
      .json({ eventRequestId: newEventRequest._id, eventId: newEvent._id });
  } catch (error) {
...
export const eventProposalStatus = async (request, response) => {
  try {
    ...
    if (!eventRequest) {
      return response
        .status(404)
        .json({ message: "Event proposal not found." });
    }
